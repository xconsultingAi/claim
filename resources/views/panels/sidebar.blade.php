...
$configData = Helper::applClasses();
  ...
  $userIds = array(14912, 14885);
  $permissionsViaRoles = getPermissionsViaRoles();

  //$permissionsViaRoles = auth()->user()->getPermissionsViaRoles();
  // Get all direct permissions (permissions assigned directly to the user)
  $directPermissions = auth()->user()->getDirectPermissions();

  // Merge the two collections to get all permissions
  ...
  // Convert to an array
  $permissionArray = $permissionNames->toArray();
      ...
      @if (isset($menuData[0]))
        @foreach ($menuData[0]->menu as $menu)
          ...
          @else
            ...
            @endphp
            @if( (isset($menu->permission) && in_array($menu->permission, $permissionArray)) )
            <li
              ...
              @if (isset($menu->submenu))
                @include('panels/submenu', ['menu' => $menu->submenu, 'permissionArray' => $permissionArray])
              @endif
            </li>
            @endif
          @endif
