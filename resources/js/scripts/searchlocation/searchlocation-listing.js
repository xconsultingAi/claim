...
//   }
//     var dtUserTable = $('.shop-type-list-table'),
//       statusObj = {
...
//        $('.shop-type-list-table thead tr').clone(true).appendTo('.shop-type-list-table thead');
//        $('.shop-type-list-table thead tr:eq(1) th').each(function (i) {
//          if (i < $('.shop-type-list-table thead tr:eq(1) th').length - 1) {
//            var title = $(this).text();
...
//         ajax: assetPath + 'searchlocation/get-searchlocation-list/' + currentLat + '/' + currentLng,
//         columns: [
...
$(function () {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      function (position) {
        // Remove 'var' to modify global variables
        currentLat = position.coords.latitude;
        currentLng = position.coords.longitude;
        console.log("Latitude: " + currentLat + ", Longitude: " + currentLng);

        // Initialize the DataTable only after getting the coordinates
        initializeDataTable();
      },
      function (error) {
        console.error("Error occurred. Error code: " + error.code);
    ...
    console.log("Geolocation is not supported by this browser.");
    // Initialize the DataTable with default or placeholder values
    initializeDataTable();
  }
      ...
      var dt_filter = dtUserTable.DataTable({
        ...
        columnDefs: [
          ...
          },
          {
            ...
            targets: 4,
            render: function (data, type, full, meta) {
              var $status = full['distance'];
              return '<span class="text-wrap">' + $status + '</span>';
            }
          },
          {
            // Actions
