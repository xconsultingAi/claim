...
$(function () {
    ...
    if (dtUserTable.length) {
        ...
        var dt_filter = dtUserTable.DataTable({
            ajax: assetPath + 'private-clinic/insurance-company/list', // JSON file to add data
            columns: [
            ...
            columnDefs: [
                ...
                },
                {
                    targets: 1,
                    render: function (data, type, full, meta) {
                        return '<span class="text-nowrap">' + full.address + '</span>';
                    }
                },
                {
                    targets: 2,
                ...
                },
                {
                    targets: 3,
                    render: function (data, type, full, meta) {
                        return '<span class="text-nowrap">' + full.mode_of_payment + '</span>';
                    }
                },
                {
                    targets: 4,
    ...
    $(document).on('click', '.delete-record', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: assetPath + 'private-clinic/insurance-company/delete/' + contact_id,
                    method: 'DELETE',
                    headers: {
    ...
    $(".add-type").click(function () {
        window.location.pathname = '/private-clinic/insurance-company/create';
    });

    $(document).on('click', '.edit-record', function (e) {
        e.preventDefault();
        var contact_id = $(this).data('contact_id');
        window.location.href = baseurl + `/private-clinic/insurance-company/${contact_id}/edit`;
    });
