...
$(function () {
    ...
    if (id != "") {
        ...
        if ($.isNumeric(id) == true) {
            ...
            $.ajax({
                url: baseurl + '/private-clinic/patient/patient-accounts/edit-invoiceData/' + patient_id + '/' + id,
                type: 'GET',
                success: function (response) {
                    if (response.status == 'success') {
                        ...
                        var invoice_date = invoiceData.invoice_date.split('-');
                        $('#invoice_date').val(invoice_date[2] + '-' + invoice_date[1] + '-' + invoice_date[0]).trigger('change');
    ...
    $('#insurance_company').on('change', function() {
        ...
        // console.log("Selected insurance company ID: " + selectedValue);

        $.ajax({
            url: baseurl + '/private-clinic/patient/patient-accounts/get-insurance-r-tax/' + selectedValue,
            type: 'GET',
            success: function(response) {
                
                var getInsuranceDetail = response.getInsuranceDetail;
                if(getInsuranceDetail.deduct_tax === 1)
                {
                    $("#tax").val(getInsuranceDetail.tax);
                }
                else if(getInsuranceDetail.deduct_tax === 0)
                {
                    $("#tax").val("0.00");
                }
                // console.log(response.getInsuranceDetail);return;
            }
        });
    });

    $("#insurance_company").trigger('change');
    ...
    if (form.length) {
        form.validate({
            ...
            submitHandler: function (form) {
                ...
                var routeforsave = 'private-clinic/patient/patient-accounts/save/';
                var routeforsaveandpay = 'private-clinic/patient/patient-accounts/saveandpay/';
                var routeforupdate = 'private-clinic/patient/patient-accounts/update/';
                var routeforupdateandpay = 'private-clinic/patient/patient-accounts/updateandpay/';
                            ...
                            Swal.fire(swalConfig).then(function (result) {
                                if (clickedButtonId === 'btn_update' || clickedButtonId === 'btn_save'){
                                    ...
                                    if(clickedButtonId === 'btn_update_pay')
                                        {
                                            window.location.href = assetPath + 'private-clinic/patient/patient-accounts/create-receipt/' + patient_id + '/' + invoice_id;
                                        }
                                        else if (clickedButtonId === 'btn_save_pay' )
                                        {
                                            // console.log(response.data.id);return;
                                             window.location.href = assetPath + 'private-clinic/patient/patient-accounts/create-receipt/' + patient_id + '/' + response.data.id;
                                        }
...
$(document).ready(function () {
    ...
    function getProcedureInfo(id) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url: baseurl + '/private-clinic/patient/patient-accounts/get-procedure-info/' + id,
                type: 'GET',
    ...
    //     var total_waived = 0;
    //     $('#procedure_table tbody tr').each(function() {
    //         var total_waivedprice = parseFloat($(this).find('td:nth-child(5)').text().replace('€ ', ''));
    //         total_waived += total_waivedprice;
    //     });
    //     return total_waived.toFixed(2);
    // }
