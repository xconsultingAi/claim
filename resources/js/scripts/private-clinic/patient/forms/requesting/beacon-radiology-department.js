...
$(function () {
    ...
    var startTimePickr = $('#time').flatpickr({
        enableTime: true,
        noCalendar: true,
        dateFormat: "H:i",
        clickOpens: true,
...
function createUpdateBeacon(formData_id, formData) {
    var beaconeUpdate = 'requesting-form/beacon-radiology-update';
    var beaconCreate = 'requesting-form/beacon-radiology-store';
        ...
        success: function (response) {
            ...
            if (response.status === 'success') {
                Swal.fire(swalConfig).then(function (result) {
                    window.location.href = baseurl + '/private-clinic/patient/requesting-form/beacon-radiology/' + formdata + '/' + id;
                });
            } else {
