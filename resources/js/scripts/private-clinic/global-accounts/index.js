...
$(function () {
  ...
  if ($('body').attr('data-framework') === 'laravel') {
    ...
    userView = assetPath + 'private-clinic/global-accounts/get/';
  }
  ...
  if (dtUserTable.length) {
    dtUserTable.DataTable({
      responsive: true,
      scrollX: true,
      // ajax: assetPath + 'private-clinic/global-accounts/get-list/', // JSON file to add data
      ajax: {
        url: assetPath + 'private-clinic/global-accounts/get-list/',
        method: 'GET',
        xhrFields: {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          responsivePriority: 2,
          render: function (data, type, full, meta) {
            var date = new Date(full.account_data.created_at);
        ...
        },
        {
          ...
          responsivePriority: 2,
          render: function (data, type, full, meta) {
            return '<span class="text-nowrap"> ' + full.account_data.patient_id ?? "" + ' </span>';
          }
        },
        {
          targets: 4,
          responsivePriority: 2,
          render: function (data, type, full, meta) {
            // console.log(full.account_data.patient);
            var firstname = (full.account_data && full.account_data.patient && full.account_data.patient.firstname) ? full.account_data.patient.firstname : "not found";
            var lastname = (full.account_data && full.account_data.patient && full.account_data.patient.lastname) ? full.account_data.patient.lastname : "not found";
            return '<span class="text-wrap"> ' + firstname  + ' ' + lastname + '</span>';
          }
        },
        {
          targets: 5,
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            return '<span class="text-nowrap">' + full['procedure_price'] + '</span>';
            // return '<span class="text-nowrap">' + full['created_at'] + '</span>';
          }
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            // console.log(full.account_data.r_tax);
            ...
            return '<span class="text-nowrap">' + payable + '</span>';
          }
        },
        {
          targets: 9,
...

        {
          ...
          render: function (data, type, full, meta) {
            ...
            return (
              ...
              (remaining_balance === 0
                ? ''
                : '<a href="' + baseurl + '/private-clinic/global-accounts/create-receipt/' + full.account_data.patient_id + '/' + full.account_data.id + '" class="dropdown-item pay" data-invoice-id="' + full.account_data.id + '" data-patient-id="' + full.account_data.patient_id + '">' +
              ...
              (remaining_balance === 0
                ...
                : (remaining_balance > 0 && remaining_balance < payable
                  ? ''
                  : '<a href="' + baseurl + '/private-clinic/global-accounts/edit-invoice/' + full.account_data.patient_id + '/' + full.account_data.id + '" class="dropdown-item pay" data-invoice-id="' + full.account_data.id + '" data-patient-id="' + full.account_data.patient_id + '">' + feather.icons['edit'].toSvg({ class: 'font-small-4 me-50' }) + 'Edit</a>'
                  +
                  '<a href="javascript:;" class="dropdown-item delete-record" data-invoice-id="' + full.account_data.id + '" data-patient-id="' + full.account_data.patient_id + '">' +
              ...
              +
              '<a href="javascript:;" class="dropdown-item invoice-print" id="invoice-print" data-invoice-id="' + full.account_data.id + '" data-patient-id="' + full.account_data.patient_id + '">' +
      ...
      createdRow: function (row, data, dataIndex) {
        ...
        var is_patient_deleted = data.account_data.patient && data.account_data.patient.deleted_at ? true : false;
        if (is_patient_deleted) {
          // Apply the class to highlight the row in light red
          $(row).addClass('highlight-patient-deleted');
        }
      },
      order: [[0, 'desc']],
      dom:
  ...
  if (receiptTable.length) {
    receiptTable.DataTable({
      // ajax: assetPath + 'private-clinic/global-accounts/get-cleared-list/', // JSON file to add data
      ajax: {
        url: assetPath + 'private-clinic/global-accounts/get-cleared-list/',
        method: 'GET',
        xhrFields: {
            withCredentials: true // Ensure cookies are sent with the request
        }
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            // var firstname = full.account_data.patient.firstname;
            // var lastname  = full.account_data.patient.lastname;
            // console.log(full.solicitor.display_name);
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '</a>' +
              '<div class="dropdown-menu dropdown-menu-end">' +
              '<a href="' + baseurl + '/private-clinic/global-accounts/edit-receipt/' + full.account_data.patient_id + '/' + full.account_data.invoice_id + '/' + full.account_data.id + '" class="dropdown-item" data-invoice-id="' + full.account_data.invoice_id + '" data-patient-id="' + full.account_data.patient_id + '" data-receipt-id="' + full.account_data.id + '">' +
              feather.icons['edit'].toSvg({ class: 'font-small-4 me-50' }) +
              'Edit</a>' +
              '<a href="javascript:;" class="dropdown-item receipt-print" data-invoice-id="' + full.account_data.invoice_id + '" data-patient-id="' + full.account_data.patient_id + '" data-receipt-id="' + full.account_data.id + '">' +
              feather.icons['printer'].toSvg({ class: 'font-small-4 me-50' }) +
              'Print</a>' +
              '<a href="javascript:;" class="dropdown-item delete-receipt" data-invoice-id="' + full.account_data.invoice_id + '" data-patient-id="' + full.account_data.patient_id + '" data-receipt-id="' + full.account_data.id + '" data-receipt-amount="' + full.account_data.amount + '">' +
              feather.icons['trash-2'].toSvg({ class: 'font-small-4 me-50' }) +
              'Delete</a>' +
              '</div>' +
              '</div>'
...
$(document).on('click', '.delete-record', function (e) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) { // Check if the user clicked 'Yes'
      ...
      $.ajax({
        url: baseurl + '/private-clinic/global-accounts/delete/' + patient_id + '/' + invoice_id,
        method: 'delete',
        headers: {
...
$(document).on('click', '.delete-receipt', function (e) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) { // Check if the user clicked 'Yes'
      var csrfToken = $('meta[name="csrf-token"]').attr('content');
      $.ajax({
        url: baseurl + '/private-clinic/global-accounts/delete-receipt/' + patient_id + '/' + invoice_id + '/' + receipt_id + '/' + receipt_amount,
        method: 'delete',
        headers: {
          'X-CSRF-TOKEN': csrfToken
        },
