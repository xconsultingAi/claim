...
$(function () {
    ...
    var dtUserTable = $('.status-definition-list-table'),
      select = $('.select2'),
      statusObj = {
    ...
    if (dtUserTable.length) {
      ...
      // $('.status-definition-list-table thead tr').clone(true).appendTo('.status-definition-list-table thead');


      $('.status-definition-list-table thead tr').clone(true).appendTo('.status-definition-list-table thead');
       $('.status-definition-list-table thead tr:eq(1) th').each(function (i) {
         if (i < $('.status-definition-list-table thead tr:eq(1) th').length - 1) {
           var title = $(this).text();
       ...
       var dt_filter = dtUserTable.DataTable({
        ajax: assetPath + 'type/status/get-status-type-list', // JSON file to add data
        columns: [
        ...
        columnDefs: [
          {
            ...
            var $branch_name = full.branch.name ? full.branch.name :  '';
          ...
          },
          {
            ...
            render: function (data, type, full, meta) {
              ...
              return '<span class="text-nowrap">' + $module + '</span>';
            }
          },
          {
            targets: 3,
            render: function (data, type, full, meta) {
              var $slug = full['slug'];
              return '<span class="text-nowrap">' + $slug + '</span>';
            }
          },
          {
            // User Status
    ...
    $(document).on('click', '.delete-record', function (e) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: assetPath + 'type/status/delete/'+ type_id,
            method: 'DELETE',
            headers: {
    ...
    $(".add-type").click(function(){
      window.location.pathname = '/type/status/create';
    }); 
    ...
    $(document).on('click', '.edit-record', function (e) {
      ...
      // Perform delete operation here
      var type_id = $(this).data('type_id');
      window.location.href = baseurl+`/type/status/${type_id}/edit`;
    });
