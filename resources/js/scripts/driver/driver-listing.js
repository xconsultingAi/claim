...
$(function () {
    ('use strict');
    var dtUserTable = $('.driver-type-list-table'),
      statusObj = {
    ...
    if (dtUserTable.length) {
       ...
       $('.driver-type-list-table thead tr').clone(true).appendTo('.vehicle-type-list-table thead');
       $('.driver-type-list-table thead tr:eq(1) th').each(function (i) {
         if (i < $('.driver-type-list-table thead tr:eq(1) th').length - 1) {
           var title = $(this).text();
      ...
      var dt_filter = dtUserTable.DataTable({
        ajax: assetPath + 'driver/get-driver-list',
        columns: [
        ...
        columnDefs: [
          ...
          
          {
            targets: 5,
            render: function (data, type, full, meta) {
              var $driving_licence_no = full['contact_person'];
  
              return '<span class="text-wrap">' + $driving_licence_no + '</span>';
            }
          },
          // {
          //   // User Status
    ...
    $(document).on('click', '.delete-record', function (e) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: assetPath + 'driver/delete/'+ type_id,
            method: 'DELETE',
            headers: {
    ...
    $(".add-type").click(function(){
      window.location.pathname = 'driver/create';
    }); 
    ...
    $(document).on('click', '.edit-record', function (e) {
      ...
      var type_id = $(this).data('type_id');
      console.log(type_id);
      window.location.href = baseurl+`/driver/${type_id}/edit`;
    });
